...
def scaled_dot_product_two_loop_single(
    ...
    similarity = torch.zeros((N_q, N_x))
    attn_weight = torch.zeros((N_q, N_x))
    out = torch.zeros((N_q, D_v))


    for i in range(N_q):
      for j in range(N_x):
        similarity[i, j] = torch.matmul(query[i], key[j].T) / D_q ** 0.5
      
      attn_weight[i] = torch.softmax(similarity[i], dim=0)

      _yi = []
      for j in range(N_x):
        _yi.append((attn_weight[i,j] * value[j]))
        
      out[i] = torch.stack(_yi).sum(dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    N, N_q, D_q = query.shape
    N, N_x, D_q = key.shape
    N, N_x, D_v = value.shape

    similarity = torch.zeros((N, N_q, N_x))
    attn_weight = torch.zeros((N, N_q, N_x))
    out = torch.zeros((N, N_q, D_v))

    for n in range(N):
      similarity[n] = torch.matmul(query[n], key[n].T) / D_q ** 0.5 
      attn_weight[n] = torch.softmax(similarity[n], dim=1)
      out[n] = torch.matmul(attn_weight[n], value[n])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    similarity = torch.matmul(query, key.transpose(2,1)) / M ** 0.5
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    attn_wegith = torch.softmax(similarity, dim=2)
    y = torch.matmul(attn_wegith, value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        layer_list = [SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)]
        self.multi_head = nn.ModuleList(layer_list)
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        outputs = []
        for att_layer in self.multi_head:
            out = att_layer(query, key, value, mask)
            outputs.append(out)
        att_out = torch.cat(outputs, dim=2)
        
        y = self.linear(att_out)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        sigma = torch.sum((x-mu)**2 / x.shape[-1], dim=-1, keepdim=True)
        x_hat = (x-mu)/torch.sqrt(sigma+self.epsilon)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multi_head = MultiHeadAttention(num_heads, emb_dim, emb_dim//num_heads)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N,K = seq.shape
    mask = torch.triu(torch.ones(1,K,K), diagonal=1)
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        attn1 = self.attention_self(dec_inp, dec_inp, dec_inp, mask)
        attn1 = self.dropout(attn1)
        attn1 = self.norm1(attn1 + dec_inp)

        attn2 = self.attention_cross(attn1, enc_inp, enc_inp)
        attn2 = self.dropout(attn2)
        attn2 = self.norm2(attn2 + attn1)

        y = self.feed_forward(attn2)
        y = self.dropout(y)
        y = self.norm3(y + attn2)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = y_.repeat(M,1).transpose(1,0).reshape(1,K,M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    for p in range(K):
        for i in range(M):
            a = math.floor(i/M)
            e = math.sin(p/10000**a + 0.5*math.pi*(i % 2))
            _y = y.clone()
            _y[0,p,i]= e
            y = _y
    ##############################################################################
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        mask = get_subsequent_mask(torch.zeros(a_emb_inp.shape[0], a_emb_inp.shape[1])).cuda()
