...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    xw = torch.matmul(x, Wx)
    hw = torch.matmul(prev_h, Wh)
    for_tanh = xw + hw + b
...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = (1 - torch.tanh(for_tanh)**2) * dnext_h

    db = dtanh.sum(dim=0)
    dprev_h = torch.matmul(dtanh, Wh.transpose(0,1))
    dWh = torch.matmul(prev_h.transpose(0,1), dtanh)
    dx = torch.matmul(dtanh, Wx.transpose(0,1))
    dWx = torch.matmul(x.transpose(0,1), dtanh)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        ...
        step_dx, dprev_h, step_dWx, step_dWh, step_db = rnn_step_backward(dout, cache[t])
        db += step_db
        dWh += step_dWh
        dWx += step_dWx
        dx[:,t,:] = step_dx
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.encoder = ImageEncoder(image_encoder_pretrained,True)
        ...
        if self.cell_type == 'lstm':
            ...
            self.projection = nn.Conv2d(input_dim, hidden_dim, (1,1))
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type != 'attn':
            feat = F.avg_pool2d(feat, kernel_size=(feat.size(2), feat.size(3)))
            feat = torch.flatten(feat, start_dim=1)
        feat = self.projection.forward(feat)

        embed = self.embedding(captions_in)
        h = self.rnn(embed, feat)
        
        output = self.fc.forward(h)

        loss = temporal_softmax_loss(output, captions_out, ignore_index=self._null)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        h = feat if self.cell_type != 'attn' else feat.mean(dim=(2,3))
        c = h if self.cell_type == 'attn' else torch.zeros_like(h)
        
        _captions = [self._start for _ in range(N)]
        for t in range(max_length):
            embed = self.embedding(_captions)
            
            if self.cell_type == 'rnn':
                h = self.rnn.step_forward(embed, h)
            elif self.cell_type == 'lstm':
                h, c = self.rnn.step_forward(embed, h, c)
            elif self.cell_type == 'attn':
                attn, _ = dot_product_attention(h, feat)
                h, c = self.rnn.step_forward(embed, h, c, attn)
            
            output = self.fc(h)
            _, next_cap = output.max(dim=1)
            _captions = next_cap
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        a = torch.matmul(x, self.Wx) + torch.matmul(prev_h, self.Wh) + self.b
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _A = A.flatten(start_dim=2)
    _attn_weights = torch.matmul(prev_h.unsqueeze(1), _A) / H ** 0.5
    
    attn_weights = F.softmax(_attn_weights, dim=2)
    attn = torch.matmul(_A, attn_weights.transpose(2,1)).squeeze(-1)
