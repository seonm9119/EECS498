...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    row_sum = margins.sum(dim=1)
    margins[range(num_train), y] -= row_sum

    dW = torch.matmul(X.T, margins) / num_train
    dW += reg * 2 * W 
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            coef = softmax[j].exp() - (1 if j == y[i] else 0)
            dW[:, j] += coef * X[i]
...
def softmax_loss_vectorized(
    ...
    loss = -log_softmax[range(num_train), y].mean()
    loss += reg * torch.sum(W*W)

    prob = log_softmax.exp()
    prob[range(num_train), y] -= 1

    dW = torch.matmul(X.T, prob) / num_train
    dW += reg * 2 * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
