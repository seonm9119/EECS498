...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = torch.matmul(x.flatten(start_dim=1), w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        dw = torch.matmul(x.flatten(start_dim=1).T, dout)
        db = dout.sum(0)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        hidden, cache_1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache_2 = Linear.forward(hidden, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dl, grads['W2'], grads['b2'] = Linear.backward(ds, cache_2)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dl, cache_1)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1: 
                ...
                hidden, cache_1 = Linear_ReLU.forward(hidden, W, b)
                hidden, cache_2 = Dropout.forward(hidden, self.dropout_param)
        ...
        for i, cache in reversed(list(enumerate(caches))):
            if i == self.num_layers - 1:  
                dl, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear.backward(dl, cache)
            
            elif self.use_dropout:
                dl = Dropout.backward(dl, cache[1]) # second one for dropout
                dl, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear_ReLU.backward(dl, cache[0])
            
            else:
                dl, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear_ReLU.backward(dl, cache)
...
def adam(w, dw, config=None):
    ...
    next_w = w - learning_rate * m_unbias / (v_unbias.sqrt() + epsilon)
