    ...
    def forward(x, w, b, conv_param):
        ...
        for i in range(H_out):
            for j in range(W_out):
                block = padded_x[:, :, i * stride : i * stride + HH, j * stride : j * stride + WW]
                blocks.append(block) 

        squeezed_x = torch.stack(blocks, dim=-1).view(N, -1, H_out*W_out)
        squeezed_out = torch.matmul(w.view(F, -1), squeezed_x) + b.unsqueeze(-1)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        pooled_x = torch.stack(blocks, dim=-1).amax(dim=(2,3))   
